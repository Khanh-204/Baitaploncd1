        ...
        <div class="col-lg-10">
            <form action="{{ route('admin.stores.update', $store->id) }}" method="POST">
                ...
                <div class="card border-0 shadow-sm rounded-4 p-4 mb-4">
                    ...
                    <div class="row g-4 mb-5">
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="city" class="form-select" required>
                                <option value="Hà Nội" {{ old('city', $store->city) == 'Hà Nội' ? 'selected' : '' }}>Hà Nội</option>
                                <option value="TP.HCM" {{ old('city', $store->city) == 'TP.HCM' ? 'selected' : '' }}>TP.HCM</option>
                                <option value="Đà Nẵng" {{ old('city', $store->city) == 'Đà Nẵng' ? 'selected' : '' }}>Đà Nẵng</option>
                                <option value="Lào Cai" {{ old('city', $store->city) == 'Lào Cai' ? 'selected' : '' }}>Lào Cai</option>
                                <option value="Vĩnh Phúc" {{ old('city', $store->city) == 'Vĩnh Phúc' ? 'selected' : '' }}>Vĩnh Phúc</option>
                            </select>
                        </div>
                        <div class="col-md-12">
                    ...
                    <div class="row g-4 mb-5">
                        <div class="col-md-6">
                            ...
                            <input type="text" name="phone" id="phone-input" class="form-control" value="{{ old('phone', $store->phone) }}" required>
                        </div>
                        <div class="col-md-6">
                            <label class="form-label fw-semibold">Giờ mở cửa <span class="text-danger">*</span></label>
                            <input type="text" name="open_time" class="form-control" value="{{ old('open_time', $store->open_time) }}" required>
                        </div>
                    </div>
                    ...
                    <div class="row g-4">
                        <div class="col-12">
                            ...
                            <input type="text" name="map_url" id="map-input" class="form-control" value="{{ old('map_url', $store->map_url) }}" required>
                        </div>
                        <div class="col-12">
                            ...
                            <div class="border rounded-4 overflow-hidden bg-light d-flex align-items-center justify-content-center" style="height: 350px;">
                                <iframe id="map-preview" src="{{ old('map_url', $store->map_url) }}" width="100%" height="100%" style="border:0;" allowfullscreen="" loading="lazy"></iframe>
                                <div id="map-placeholder" class="text-muted fw-medium" style="display: none;">
                ...
                <div class="text-end mb-5">
                    <button type="submit" class="btn btn-primary px-5 py-3 fw-bold rounded-pill shadow-sm fs-5">
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        mapInput.addEventListener('input', function() {
            ...
            if (!val) {
                ...
                mapPlaceholder.innerHTML = `<i class="bi bi-geo-alt display-4 d-block text-center mb-2"></i> Bản đồ sẽ hiển thị tại đây khi Sếp dán link`;
            ...
            if(val.includes('google.com/maps/embed') || val.includes('embed')) {
                mapPreview.src = val;
                mapPreview.style.display = 'block';
                mapPlaceholder.style.display = 'none';
            } else {
                mapPreview.style.display = 'none';
                mapPlaceholder.innerHTML = `<div class="text-danger text-center"><i class="bi bi-exclamation-triangle-fill display-4 d-block mb-2"></i><h6 class="fw-bold">LỖI: ĐÂY KHÔNG PHẢI LINK NHÚNG (EMBED)</h6></div>`;
                mapPlaceholder.style.display = 'block';
            }
        });
